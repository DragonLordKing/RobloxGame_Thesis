...
Class: ModuleScript
Original path: game.StarterPlayer.StarterPlayerScripts.PlayerModule.CameraModule.VRVehicleCamera
Exported from: MMORPG Testing
Original comments: removed
Children: 0
Properties: Archivable=false, LinkedSource=""
Services: Players, RunService, VRService
Requires:
...
Functions: pitchVelocity, yawVelocity, NormalizeAngle, VRVehicleCamera.new, VRVehicleCamera:Reset, VRVehicleCamera:_StepRotation, VRVehicleCamera:_GetThirdPersonLocalOffset, VRVehicleCamera:_GetFirstPersonLocalOffset, VRVehicleCamera:Update, VRVehicleCamera:addDrift, VRVehicleCamera:UpdateRotationCamera, VRVehicleCamera:UpdateStepRotation, VRVehicleCamera:UpdateComfortCamera
Clean source lines: 505
]]
...
function VRVehicleCamera:UpdateStepRotation(dt)
	...
	if math.abs(yawDelta) > 0 then
		...
		if not UserGameSettings.VRSmoothRotationEnabled then
			...
			local headOffsetCurrent = focusWithRotation:ToObjectSpace(cf * headOffset)
			local currentVector = Vector3.new(headOffsetCurrent.X, 0, headOffsetCurrent.Z).Unit
			local currentAngleFromBack = math.acos(currentVector:Dot(Vector3.new(0, 0, 1)))

			local headOffsetRotated = focusWithRotation:ToObjectSpace(rotatedCamera * headOffset)
			local rotatedVector = Vector3.new(headOffsetRotated.X, 0, headOffsetRotated.Z).Unit
